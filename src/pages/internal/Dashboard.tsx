            ...
            <div className="lg:w-10/12 w-11/12 p-8 bg-white floating-shadow-md mx-auto mt-6 rounded-lg">
                <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4">
                    <Panel
                        ...
                        count={laporanCount.find((val) => val.id === STATUS_LAPORAN.SEMUA_KASUS)?.totalCase}
                    />
                    <Panel
                        title="Total Kasus Menunggu Validasi"
                        date={new Date().toISOString()}
                        count={laporanCount.find((val) => val.id === STATUS_LAPORAN.MENUNGGU_VALIDASI)?.totalCase}
                    />
                    <Panel
                        title="Total Kasus Selesai"
                        date={new Date().toISOString()}
                        count={laporanCount.find((val) => val.id === STATUS_LAPORAN.KASUS_SELESAI)?.totalCase}
                    />
                    <Panel
                        title="Total Kasus Diteruskan ke DP3A"
                        date={new Date().toISOString()}
                        count={laporanCount.find((val) => val.id === STATUS_LAPORAN.KASUS_DITERUSKAN)?.totalCase}
                    />
                    <Panel
